...
;EVEN USING EXAMPLE CODE FROM THE SOLUTIONS, THSI CAUSED A CONVERSION ISSUE, SO I'M JUST GOING TO LEAVE MY OWN CODE HERE/
...
; test
;(koch-snowflake (cons 100 100) 12 5)

;;; Part 3: L-systems w/ Turtle Graphics

; apply-rule

(define (apply-rule str r)
  (foldr (lambda (ir buf) (if (eq? str (car ir))
                                (cdr ir)
                               buf))
         (string str)
         r))
...
(define (l-system-step str reg)
  (let* ([bup (string->list str)])
    ;(string-append "" (foldr (apply-rule str rules) bup))))  this gives a contract vio
    (foldr (lambda (stra buf) (string-append (apply-rule stra reg) buf)) ""  bup))) 

; test
;(l-system-step "F" (list (cons #\F "FF")))           ; => "FF"
;(l-system-step "F+F" (list (cons #\F "FF")))         ; => "FF+F;F"
...
(define (l-system-generate str reg iter)
  (if (zero? iter)
             str
             ;string-append str (l-system-generate str reg (- iter 1))
             (l-system-generate (l-system-step str reg) reg (- iter 1))))
...
(define (interpert-l-system str turtle step-size turn-angle stack background)
  ;(if (empty? str) (begin (print"empty") background)
  (if (= (string-length str) 0) (begin background)
  (let* ([currChar (string-ref str 0)]
         [restStr (substring str 1)])
    (cond
      ((or (eq? currChar #\G) (eq? currChar #\F))
       ;(let* ([newturt (turtle-forward turtle step-size)])
        ; (interpert-l-system restStr newturt step-size turn-angle stack (draw-line (turtle-point turtle) (turtle-point newturt) "red" background))))
       ;(print "forward")
       (interpert-l-system restStr (turtle-forward turtle step-size) step-size turn-angle stack (draw-line (turtle-point turtle) (turtle-point (turtle-forward turtle step-size)) "black" background)))
      ((eq? currChar #\+)
       ;(print "plus")
       (interpert-l-system restStr (turtle-turn turtle turn-angle) step-size turn-angle stack background))
      ((eq? currChar #\-)
       ;(print "minus")
       (interpert-l-system restStr (turtle-turn turtle (- turn-angle)) step-size turn-angle stack background))
      ((eq? currChar #\[)
       ;(print"lbracket")
       (interpert-l-system restStr turtle step-size turn-angle (cons turtle stack) background))
      ((eq? currChar #\])
       ;(print "rbracket")
       (interpert-l-system restStr (car stack) step-size turn-angle (cdr stack) background))
      (interpert-l-system restStr turtle step-size turn-angle stack background)))))
        

(define (draw-l-system axiom rules iterations step-size turn-angle x y a)
  (let ((str (l-system-generate axiom rules iterations))) ;(begin (print str)
    (interpert-l-system str (make-turtle x y a) step-size turn-angle '() (rectangle 300 300 "solid" "white"))));)
...
; plant
;(define plant
 ; (draw-l-system "X" THIS CODE KILLS DRRACKET
...
(draw-fatou-tr (lambda (z) (+ (* (cos z) z) c)) 10 800 800) 
